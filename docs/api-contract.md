...
- **タイムゾーン**: 「今日」の判定はユーザー設定のタイムゾーンで行う。
  **`users.timezone` に IANA 名を保存する方式に決定**（2026-08-09）。`X-Timezone` ヘッダは採らない。
  端末ごとにヘッダが変わると、同じ日の記録が別の日に落ちて連続日数が狂うため。
  週の起点は月曜（ISO 8601）に固定する。詳細は `docs/db-design.md` §2.3

## 3. 認証：**案 B（JWT）に決定**（2026-08-09）

アクセストークン（JWT・ステートレス）+ リフレッシュトークン（DB 保存・ローテーション）で実装する。
リフレッシュトークンは平文を保存せず SHA-256 のハッシュで照合し、
使用済みトークンの再提示を検知したら系列ごと失効させる。
スキーマは `docs/db-design.md` §5.2（`refresh_tokens`）。

未決なのはアクセストークンとリフレッシュトークンの寿命、
およびパスワードハッシュのアルゴリズム（bcrypt / argon2id）。どちらもスキーマには影響しない。
...
>
> **結論としては案 B を採用した。** 将来ネイティブアプリを出す前提を優先したため。
> 案 B の弱点である「失効の難しさ」は、アクセストークンを短命にし、
> リフレッシュトークンを DB でローテーション管理することで抑える。
...
1. ~~**認証方式**（案 A / B / C）~~ — **案 B（JWT）に決定**（2026-08-09）。§3 参照
...
3. ~~**データベース**~~ — **MySQL に決定**（2026-08-09）。アクセスは `database/sql` +
   ...
   規約は `.claude/skills/backend-conventions`、スキーマ設計は `docs/db-design.md`
4. **画像ストレージ** — ギャラリーとスケッチの実画像をどこに置くか（S3 / R2 / Cloud Storage）
5. ~~**将来ネイティブアプリを出す予定があるか**~~ — 案 B（JWT）採用の判断材料として解決済み
